...
| [`src/server/schemas.ts`](src/server/schemas.ts) | `WorkspacePickSchema` (single-repo: `workspaceRoot`), `RootPickSchema` (fan-out: polymorphic `root`), **`MAX_ROOT_PATHS`** (256) |
| [`src/server/inventory.ts`](src/server/inventory.ts) | `InventoryEntryJson`, `MAX_INVENTORY_ROOTS_DEFAULT`, `MAX_BRANCH_STATUS_LINES_DEFAULT`, `makeSkipEntry`, `collectInventoryEntry` |
