...
| [`src/server/git-grep-tool.ts`](src/server/git-grep-tool.ts) | `git_grep` — read-only fan-out content search (`git grep -n`); optional `ref` for tree-at-commit search, `filesOnly` for path-only listing; optional `pickaxe` for `git log -S`/`-G` history (`commits[]`) |
